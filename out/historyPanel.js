...
class HistoryPanel {
        ...
        this.panel.webview.onDidReceiveMessage(async (msg) => {
            try {
                ...
                if (msg?.type === "bulkRevertFolder") {
                    ...
                    await (0, bulkRevert_1.runBulkRevertForFolder)({
                        ctx: this.context,
    ...
    async refresh() {
        ...
        let snaps = [];
        try {
            snaps = await (0, apiClient_1.getLatestSnapshots)(this.installationId, 400);
        }
        catch (err) {
    ...
    async openDiff(snapshotId, filePathAbs) {
        const leftUri = vscode.Uri.parse(`devtracker-snapshot:/${snapshotId}`);
        const rightUri = vscode.Uri.file(filePathAbs);
        await vscode.commands.executeCommand("vscode.diff", leftUri, rightUri, `DevTracker Diff: ${this.prettifyAbsToRel(filePathAbs)}`);
        try {
            await vscode.commands.executeCommand("workbench.action.compareEditor.nextChange");
        }
        catch { }
    }
    async revertToSnapshot(snapshotId, filePathAbs) {
        const ok = await vscode.window.showWarningMessage(`Revert "${this.prettifyAbsToRel(filePathAbs)}" to snapshot #${snapshotId}?\n\nTip: You can undo with Ctrl+Z.`, { modal: true }, "Revert");
        if (ok !== "Revert")
            return;
