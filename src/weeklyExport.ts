 ...
 */
function guessProjectRoot(absPath: string) {
...
function intensity(totalSnapshots: number) {
  if (totalSnapshots <= 5) return "Quick touch";
  if (totalSnapshots <= 20) return "Steady sessions";
  return "Deep work";
}

function detectNatureFromAbs(files: { path: string; snapshots: number }[]) {
  let tests = 0;
  let docs = 0;
  let config = 0;
  let code = 0;
  ...
  for (const f of files) {
    ...
    if (p.indexOf("/test") >= 0 || p.indexOf(".spec.") >= 0 || p.indexOf(".test.") >= 0) tests += w;
    if (ext === "md") docs += w;
    if (
      ext === "json" ||
      ext === "yml" ||
...
function netProgressHint(nature: string, filesTouched: number) {
  if (nature.indexOf("testing") >= 0) return "Confidence & stability improved";
  if (nature.indexOf("configuration") >= 0) return "Wiring & integration progressed";
  if (nature.indexOf("documentation") >= 0) return "Clarity & handoff improved";
  if (filesTouched >= 20) return "Broad cleanup & forward momentum";
  if (filesTouched <= 5) return "Small but meaningful progress";
  return "Steady progress & cleanup";
}

function buildWebviewHtml(nonce: string) {
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8"/>
...
async function openRangeReflectionWebview(installationId: string, mode: RangeMode) {
  const now = new Date();
  const daysBack = mode === "week" ? 6 : 29;
  const start = startOfDay(addDays(now, -daysBack));
  const end = addDays(startOfDay(now), 1);

  const snaps: SnapshotSummary[] = await getLatestSnapshots(installationId, 20000);
  ...
  const byRoot = new Map<string, { count: number; files: Map<string, number> }>();
  for (const [abs, count] of byFile.entries()) {
    const root = guessProjectRoot(abs);
    const cur = byRoot.get(root) ?? { count: 0, files: new Map<string, number>() };
    cur.count += count;
  ...
  const panel = vscode.window.createWebviewPanel(
    mode === "week" ? "devtracker.weeklyReflection" : "devtracker.monthlyReflection",
    `DevTracker — ${title}`,
...
export async function exportWeeklyInsight(installationId: string) {
  return openRangeReflectionWebview(installationId, "week");
}

export async function exportMonthlyInsight(installationId: string) {
