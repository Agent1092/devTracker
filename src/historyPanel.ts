...
function relToBase(abs: string, base: string) {
...
export class HistoryPanel {
    ...
    this.panel.webview.onDidReceiveMessage(async (msg) => {
      try {
        ...
        if (msg?.type === "revert") {
          ...
          if (String(msg.scope) === "global") {
            vscode.window.showInformationMessage(
              "DevTracker: Revert is only available in Local History."
            );
            return;
          }
          await this.revertToSnapshot(
            Number(msg.snapshotId),
            String(msg.filePath)
          );
          return;
        }
        ...
        if (msg?.type === "bulkRevertFolder") {
          if (String(msg.scope) === "global") {
            vscode.window.showInformationMessage(
              "DevTracker: Bulk Revert is only available in Local History."
            );
            return;
          }
  ...
  private async refresh() {
    ...
    try {
      snaps = await getLatestSnapshots(this.installationId, 1600);
    } catch (err: any) {
      this.panel.webview.postMessage({
        type: "error",
   ...
   */
  private async bulkRevertLastSession(folderRel: string) {
    const wsRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    if (!wsRoot) return;

    const last = getLastSessionWindow(this.context);
    if (!last) {
    ...
    let snaps: SnapshotSummary[] = [];
    try {
      snaps = await getLatestSnapshots(this.installationId, 2000);
    } catch (e: any) {
    ...
    function underFolder(abs: string) {
      const a = normalize(abs);
    ...
    for (const s of snaps) {
      ...
      if (!underFolder(s.file_path)) continue;
    ...
    if (!restorePlan.length) {
      vscode.window.showInformationMessage(
        `DevTracker: No pre-session snapshots available to revert under "${folderRel || "(root)"}".`
      );
      return;
    }

    const confirm = await vscode.window.showWarningMessage(
      `Bulk Revert will overwrite ${restorePlan.length} file(s) under "${folderRel || "(root)"}". Continue?`,
      { modal: true },
      "Revert"
    );
